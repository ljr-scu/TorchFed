...
class DealDataset(Dataset):
    ...
    def __init__(self, folder, data_name, label_name, transform=None):
        (train_set, train_labels) = self.load_data(folder, data_name,
...
class TorchEMNIST(TorchDataset):
    num_classes = 26
    ...
    def load_user_dataset(self) -> List[List[TorchUserDataset]]:
        ...
        train_dataset = DealDataset(r'D:\torchfed-master\torchfed-master\example\data\EMNIST\raw', "emnist-letters-train-images-idx3-ubyte.gz",
                                    "emnist-letters-train-labels-idx1-ubyte.gz", transform=self.transform)
        test_dataset = DealDataset(r'D:\torchfed-master\torchfed-master\example\data\EMNIST\raw', "emnist-letters-test-images-idx3-ubyte.gz",
                                   "emnist-letters-test-labels-idx1-ubyte.gz", transform=self.transform)
        # train_dataset = torchvision.datasets.EMNIST(
        #     self.root, split='letters', train=True, transform=self.transform, target_transform=self.target_transform,
        #     download=self.download)
        # test_dataset = torchvision.datasets.EMNIST(
        #     self.root, split='letters', train=False, transform=self.transform, target_transform=self.target_transform,
        #     download=self.download)
        ...
        props = np.random.lognormal(
            0,2.,(self.num_classes,self.num_users,self.num_labels_for_users)
        )
        props = np.array([[[len(v) - self.num_users]] for v in split_inputs]) * props / np.sum(props,(1,2),keepdims=True)
        ...
        for user_idx in trange(self.num_users):
            ...
            user_x[user_idx] = torch.Tensor(user_x[user_idx]).view(-1,num_channels,num_width,num_height).type(torch.float32)
    ...
    def load_global_dataset(self) -> List[TorchGlobalDataset]:
        train_dataset = torchvision.datasets.EMNIST(
            self.root, split='byclass', train=True,transform=self.transform, target_transform=self.target_transform, download=self.download)
        test_dataset = torchvision.datasets.EMNIST(
            self.root, split='byclass',train=False, transform=self.transform, target_transform=self.target_transform, download=self.download)
        return [
